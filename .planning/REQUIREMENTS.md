...
- [x] **SEC-04**: Every ADO comment the system posts goes through a sanitizing formatter and carries the `<!-- [automated-agent] -->` loop-shield marker — including the 10 inline comments in execute/rework workers (H3; `src/execute/worker.ts:96,159,172,187,219`, `src/execute/rework-worker.ts:146,169,211,226,242,276`)
- [x] **SEC-05**: Git push failure in production fails the ticket (Blocked + dedup `failed` + alert comment); error-swallow exists only under `NODE_ENV=test` (H4; `src/execute/worker.ts:267-271`, `src/execute/repair.ts:71-75`)
